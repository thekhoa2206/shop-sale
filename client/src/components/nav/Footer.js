      ...
      <Box style={{width: "60%", margin: "auto", padding: 10}}>
        ...
        <Typography key={index} style={{textDecoration: "underline", cursor: "pointer"}} onClick={() => {window.location.href = `/category/${item.slug}`}}>
          ...
          <Grid xs={4} item> 
            ...
            <TextField placeholder='input email register' value={email}
        onChange={(e) => setEmail(e.target.value)} autoFocus type="email"/>
            <Button style={{height: 36}} onClick={(e) => {handleSubmit(e)}}>Register</Button>
          </Grid>
        </Grid>
      </Box>
    </Box>
